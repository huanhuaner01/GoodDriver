...
public class CalendarGridView extends ViewGroup {
	...
	public CalendarGridView(Context context, AttributeSet attrs) {
		super(context, attrs);
		dividerPaint.setColor(getResources().getColor(
				R.color.calendar_cell_border));
	}

	@Override
	public void addView(View child, int index, ViewGroup.LayoutParams params) {
		/*
		 * if (getChildCount() == 0) { ((CalendarRowView)
		 * child).setIsHeaderRow(true); }
		 */
	...
	protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
		...
		for (int c = 0, numChildren = getChildCount(); c < numChildren; c++) {
			...
			if (child.getVisibility() == View.VISIBLE) {
				/*
				 * if (c == 0) { // It's the header: height should be
				 * wrap_content. measureChild(child, rowWidthSpec,
				 * makeMeasureSpec(cellSize, AT_MOST)); } else {
				 */
		...
		final int measuredWidth = widthMeasureSize; // Fudge factor to make the
													// borders show up.
		setMeasuredDimension(measuredWidth, (int) (totalHeight * 1.2));
	...
	protected void onLayout(boolean changed, int left, int top, int right,
		...
		for (int c = 0, numChildren = getChildCount(); c < numChildren; c++) {
			final View child = getChildAt(c);
			final int rowHeight = (int) (child.getMeasuredHeight() * 1.2);
